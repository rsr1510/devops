...
EOF
                    ...
                    sh '''
                        scp -i $SSH_KEY -o StrictHostKeyChecking=no deploy.sh ubuntu@$EC2_HOST:/tmp/deploy.sh
                        ssh -i $SSH_KEY -o StrictHostKeyChecking=no ubuntu@$EC2_HOST 'bash /tmp/deploy.sh'
                    '''
                }
            }
        }
